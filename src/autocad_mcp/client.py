...
async def get_backend() -> AutoCADBackend:
    ...
    async with _init_lock:
        ...
        elif backend_name == "com":
            from autocad_mcp.backends.com_backend import COMBackend
            backend = COMBackend()
        else:
...
async def switch_backend(backend_name: str) -> AutoCADBackend:
    """Force-switch to a specific backend. Used by connection management."""
    global _backend
    async with _init_lock:
        if backend_name == "file_ipc":
            from autocad_mcp.backends.file_ipc import FileIPCBackend
        ...
        _backend = backend
        log.info("backend_switched", backend=_backend.name)
        return _backend
...
def _error(e: Exception, context: str = "") -> str:
    ...
    if "window not found" in msg_lower or "no autocad" in msg_lower:
        hint = "AutoCAD is not running or no drawing is open. Start AutoCAD and open a .dwg file."
    ...
    elif "com_error" in msg_lower or "com" in msg_lower:
        hint = "COM error. Ensure the CAD application is running and accessible."
    else:
